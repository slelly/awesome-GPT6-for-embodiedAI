...
def main() -> int:
    ...
    with sync_playwright() as pw:
        ...
        assert page.locator('.card').count() == 42
        ...
        assert sum(item['status'] == 'verified' for item in publication_dates.values()) == 54
        ...
        assert len(expected['projects']) == 42 and len(expected['social']) == 15
        ...
        assert page.locator('.card').evaluate_all('(els) => els.map((el) => el.dataset.projectId)') == ['P42', 'P11', 'P10', 'P08', 'P09']
        ...
        assert actual_dual == ['P37', 'P35', 'P06', 'P16', 'P14', 'P15', 'P39', 'P13'], actual_dual
        page.locator('#clear-query').evaluate('(element) => element.click()')
